...
def hacer_prediccion_emilia_romagna_2025(datos_piloto, formula1_predict, formula_1, drivers, circuits):
    try:
        ...
        # Calcular puntos esperados basados en las predicciones
        ...
        prediccion_2025 = {
            ...
            'Promedio de Vueltas': round(datos_similares['laps'].mean(numeric_only=True), 1),
            'Temperatura Promedio': 25,
            'Humedad Promedio': 65,
            'Velocidad Viento Promedio': 5
        }
        ...
        print(f"Error al hacer predicción para Emilia-Romagna 2025: {str(e)}")
        raise

def hacer_prediccion_emilia_romagna_2025_albon(datos_piloto, formula1_predict, formula_1, drivers, circuits):
    try:
        # Obtener datos de circuitos similares
        datos_similares = datos_piloto.copy()
...
def crear_grafico_outliers_lap_times(driver_id, lap_times, races):
    try:
        print(f"Creando gráfico de outliers para tiempos de vuelta del piloto {driver_id}")
        
        # Filtrar tiempos de vuelta del piloto
        piloto_lap_times = lap_times[lap_times['driverId'] == driver_id].copy()
        
        if piloto_lap_times.empty:
            print(f"No hay datos de tiempos de vuelta para el piloto con ID {driver_id}")
            return None
        
        # Convertir milliseconds a numérico
        piloto_lap_times['milliseconds'] = pd.to_numeric(piloto_lap_times['milliseconds'], errors='coerce')
        
        # Unir con información de carreras
        piloto_lap_times = pd.merge(piloto_lap_times, races[['raceId', 'year', 'name']], on='raceId', how='left')
        
        # Filtrar datos desde 2019
        piloto_lap_times = piloto_lap_times[piloto_lap_times['year'] >= 2019]
        
        if piloto_lap_times.empty:
            print(f"No hay datos recientes de tiempos de vuelta para el piloto con ID {driver_id}")
            return None
        
        # Crear gráfico de caja (box plot) para visualizar outliers
        fig = go.Figure()
        
        # Agrupar por año y carrera
        for year in piloto_lap_times['year'].unique():
            year_data = piloto_lap_times[piloto_lap_times['year'] == year]
            
            # Calcular estadísticas
            q1 = year_data['milliseconds'].quantile(0.25)
            q3 = year_data['milliseconds'].quantile(0.75)
            iqr = q3 - q1
            ...
            # Filtrar outliers
            outliers = year_data[
                (year_data['milliseconds'] < lower_bound) | 
                (year_data['milliseconds'] > upper_bound)
            ]
            
            # Agregar box plot
            fig.add_trace(go.Box(
                y=year_data['milliseconds'],
                name=str(year),
                boxpoints='outliers',
                marker=dict(color='#FF5733'),
                line=dict(color='#FF5733'),
                hovertemplate="Año: %{x}<br>Tiempo: %{y:.2f} ms<br>Extra: %{text}",
                text=year_data['name']
            ))
        
        # Calcular estadísticas generales
        media_tiempo = piloto_lap_times['milliseconds'].mean()
        mejor_tiempo = piloto_lap_times['milliseconds'].min()
        peor_tiempo = piloto_lap_times['milliseconds'].max()
        
        fig.update_layout(
            title=f'Tiempos de vuelta y outliers (2019-2024)<br>Media: {media_tiempo:.2f} ms | Mejor: {mejor_tiempo:.2f} ms | Peor: {peor_tiempo:.2f} ms',
            xaxis_title='Año',
            ...
            yaxis=dict(
                showgrid=True,
                gridcolor='rgba(255, 255, 255, 0.1)',
                range=[mejor_tiempo * 0.95, peor_tiempo * 1.05]
            ),
            showlegend=False
        )
...
def crear_grafico_outliers_pit_stops(driver_id, pit_stops, races):
    try:
        print(f"Creando gráfico de outliers para paradas en boxes del piloto {driver_id}")
        
        # Filtrar datos de paradas en boxes del piloto
        piloto_pit_stops = pit_stops[pit_stops['driverId'] == driver_id].copy()
        ...
        # Convertir milliseconds a numérico
        piloto_pit_stops['milliseconds'] = pd.to_numeric(piloto_pit_stops['milliseconds'], errors='coerce')
        
        # Unir con información de carreras
        piloto_pit_stops = pd.merge(piloto_pit_stops, races[['raceId', 'year', 'name']], on='raceId', how='left')
        
        # Filtrar datos desde 2019
        piloto_pit_stops = piloto_pit_stops[piloto_pit_stops['year'] >= 2019]
        
        if piloto_pit_stops.empty:
            print(f"No hay datos recientes de paradas en boxes para el piloto con ID {driver_id}")
            return None
        ...
        # Agrupar por año y carrera
        for year in piloto_pit_stops['year'].unique():
            year_data = piloto_pit_stops[piloto_pit_stops['year'] == year]
        ...
        # Calcular estadísticas generales
        media_tiempo = piloto_pit_stops['milliseconds'].mean()
        mejor_tiempo = piloto_pit_stops['milliseconds'].min()
        peor_tiempo = piloto_pit_stops['milliseconds'].max()
        
        fig.update_layout(
            title=f'Paradas en boxes y outliers (2019-2024)<br>Media: {media_tiempo:.2f} ms | Mejor: {mejor_tiempo:.2f} ms | Peor: {peor_tiempo:.2f} ms',
            xaxis_title='Año',
...
def crear_modelos(results, drivers, circuits, formula_1):
    try:
        ...
        # Definir las características necesarias para el modelo
        required_features = [
            'driverId', 'constructorId', 'grid', 'laps', 'circuitId', 'length', 'turns',
            'Constructor Experience', 'Driver Experience', 'Driver Age', 'Driver Wins',
            'Constructor Wins', 'Driver Constructor Experience', 'DNF Score', 'prev_position'
        ]
        ...
        for col in required_features:
            if col not in formula_1.columns:
                if col in ['Constructor Experience', 'Driver Experience', 'Driver Age',
                    ...
                    formula_1[col] = 0
                elif col in ['grid', 'laps']:
                    formula_1[col] = 10  # Valor medio típico
                elif col in ['length', 'turns']:
                    formula_1[col] = 3.337 if col == 'length' else 19  # Valores típicos de Mónaco
        ...
        formula1_predict = RandomForestClassifier(
            n_estimators=100,
            max_depth=10,
            min_samples_split=5,
            min_samples_leaf=2,
...
def hacer_prediccion(driver_name, grid, circuit_loc, formula1_predict, formula_1, drivers, circuits):
    try:
        ...
        features = {
            ...
            'grid': grid,
            'laps': circuit['laps'] if 'laps' in circuit else 78,  # Valor por defecto para Mónaco
            'circuitId': circuit['circuitId'],
            'length': circuit['length'] if 'length' in circuit else 3.337,  # Longitud típica de Mónaco
            'turns': circuit['turns'] if 'turns' in circuit else 19,  # Curvas típicas de Mónaco
            ...
            'prev_position': input_data.get('prev_position', 10)
        }
        ...
        required_columns = formula1_predict.feature_names_in_
...
def crear_grafico_predicciones_williams_emilia_romagna(datos_williams, formula1_predict, formula_1, drivers, circuits):
    try:
        print("Creando gráfico de predicciones para Williams en Emilia Romagna")
        
        # Obtener datos de circuitos similares
        circuitos_similares = circuits[
            circuits['country'].isin(['Italy', 'Germany', 'France', 'Spain', 'Belgium', 'Austria'])
        ]['circuitId'].tolist()
        
        # Filtrar datos por circuitos similares
        datos_similares = datos_williams[datos_williams['circuitId'].isin(circuitos_similares)].copy()
        
        if len(datos_similares) < 5:
            print("No hay suficientes datos históricos para hacer una predicción confiable")
            return None
        ...
        for col in features:
            if col not in datos_similares.columns:
                if col in ['Constructor Experience', 'Constructor Wins']:
                    datos_similares[col] = 0
                elif col in ['AirTemp', 'Humidity', 'WindSpeed']:
                    datos_similares[col] = 25.0 if col == 'AirTemp' else 65.0 if col == 'Humidity' else 5.0
                elif col == 'pit_stops':
        ...
        X = datos_similares[features]
        y = datos_similares['points']
        
        # Escalar los datos
        scaler = StandardScaler()
        X_scaled = scaler.fit_transform(X)
        
        # Crear y entrenar modelos de regresión
        from sklearn.ensemble import RandomForestRegressor
        ...
        ultimos_datos = datos_similares.tail(5).mean(numeric_only=True)
        
        X_pred = pd.DataFrame([{
            'grid': 8,
            'laps': 78,
            'turns': 19,
            'Constructor Experience': float(ultimos_datos['Constructor Experience'] + 1),
            'Constructor Wins': float(ultimos_datos['Constructor Wins']),
            'AirTemp': 25.0,
            'Humidity': 65.0,
            'WindSpeed': 5.0,
            'milliseconds': float(ultimos_datos['milliseconds']),
            'pit_stops': float(ultimos_datos['pit_stops'])
        }])
        
        X_pred_scaled = scaler.transform(X_pred)
        
        # Hacer predicciones
        rf_pred = float(rf_model.predict(X_pred_scaled)[0])
        svr_pred = float(svr_model.predict(X_pred_scaled)[0])
        knn_pred = float(knn_model.predict(X_pred_scaled)[0])
        
        # Calcular confianza basada en el error cuadrático medio
        from sklearn.metrics import mean_squared_error
        import numpy as np
        
        rf_conf = 1 / (1 + mean_squared_error(y, rf_model.predict(X_scaled)))
        svr_conf = 1 / (1 + mean_squared_error(y, svr_model.predict(X_scaled)))
        knn_conf = 1 / (1 + mean_squared_error(y, knn_model.predict(X_scaled)))
        
        # Crear gráfico
        fig = go.Figure()
        
        # Agregar barras para cada modelo
        modelos = ['Random Forest', 'SVR', 'KNN']
        predicciones = [rf_pred, svr_pred, knn_pred]
        confianzas = [rf_conf, svr_conf, knn_conf]
        
        fig.add_trace(go.Bar(
            x=modelos,
            y=predicciones,
            text=[f'{c:.1f}%' for c in confianzas],
            textposition='auto',
            marker_color=['#FF5733', '#33FF57', '#3357FF'],
            hovertemplate="Modelo: %{x}<br>Puntos: %{y:.1f}<br>Confianza: %{text}"
        ))
        
        # Calcular estadísticas
        media_prediccion = sum(predicciones) / len(predicciones)
        mejor_prediccion = max(predicciones)
        peor_prediccion = min(predicciones)
        
        fig.update_layout(
            title=f'Predicciones de puntos para Williams en Emilia Romagna GP 2025<br>Media: {media_prediccion:.1f} | Mejor: {mejor_prediccion:.1f} | Peor: {peor_prediccion:.1f}',
            xaxis_title='Modelo',
        ...
        print(f"Error al crear gráfico de predicciones para Williams: {str(e)}")
        return None
...
def index():
    try:
        ...
        try:
            ...
            # Crear gráfico de predicciones para Williams
            print("Creando gráfico de predicciones para Williams...")
            grafico_predicciones_williams = crear_grafico_predicciones_williams_emilia_romagna(
                datos_williams, formula1_predict, formula_1, drivers, circuits
            )
        except Exception as e:
        ...
        try:
            ...
            # Hacer predicciones
            prediccion_sainz = hacer_prediccion('sainz', 5, 'Imola', formula1_predict, formula_1, drivers, circuits)
            prediccion_albon = hacer_prediccion('albon', 8, 'Imola', formula1_predict, formula_1, drivers, circuits)
            
            # Hacer predicción específica para Emilia-Romagna 2025
            prediccion_emilia_romagna_2025_sainz = hacer_prediccion_emilia_romagna_2025(datos_sainz, formula1_predict, formula_1, drivers, circuits)
            prediccion_emilia_romagna_2025_albon = hacer_prediccion_emilia_romagna_2025_albon(datos_albon, formula1_predict, formula_1, drivers, circuits)
        except Exception as e:
            print(f"Error al hacer predicciones: {str(e)}")
            raise
        
        # Obtener los nombres de los features
        feature_names = list(formula_1.drop(['position', 'seconds', 'podium', 'date', 'fastestLapSpeed', 'raceId'], axis=1).columns)
        ...
        return render_template('index.html',
                             ...
                             grafico_predicciones_williams=json.dumps(grafico_predicciones_williams, cls=plotly.utils.PlotlyJSONEncoder),
                             ...
                             prediccion_sainz=prediccion_sainz,
                             prediccion_albon=prediccion_albon,
                             prediccion_emilia_romagna_2025_sainz=prediccion_emilia_romagna_2025_sainz,
                             prediccion_emilia_romagna_2025_albon=prediccion_emilia_romagna_2025_albon,
